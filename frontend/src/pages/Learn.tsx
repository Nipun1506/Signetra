    ...
    <div className="pt-24 px-10 pb-12 max-w-7xl mx-auto w-full">
      ...
      <div className="flex justify-between items-end mb-10">
        ...
        <div className="flex gap-1 bg-surface-container-low p-1 rounded-xl">
          <button className="px-6 py-2 bg-surface-container-highest text-primary font-semibold rounded-lg text-sm transition-all">Symbol Tutorials</button>
          <button className="px-6 py-2 text-on-surface-variant hover:text-on-surface font-medium rounded-lg text-sm transition-all">Video Library</button>
        </div>
      </div>

      {/* Symbol Tutorials Grid */}
      ...
      {/* Video Library */}
      <section className="mt-20 border-t border-outline-variant/10 pt-16">
      ...
      <div className="fixed bottom-8 right-8 z-50">
        <button className="bg-gradient-to-br from-[#adc6ff] to-[#4d8eff] w-16 h-16 rounded-full shadow-2xl shadow-primary/30 flex items-center justify-center group">
          <span className="material-symbols-outlined text-on-primary text-3xl group-hover:rotate-12 transition-transform">emoji_events</span>
          <div className="absolute -top-2 -right-2 bg-on-background text-[#00285d] text-[10px] font-extrabold w-6 h-6 rounded-full flex items-center justify-center border-2 border-primary">72%</div>
        </button>
      </div>
    </div>
  )
