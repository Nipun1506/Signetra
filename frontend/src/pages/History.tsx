...
/** Maps gesture phrase to a material-symbols icon name. */
function getIcon(phrase: string): string {
  ...
  const tutorialIcon = TUTORIALS.find(
    t => t.phrase.toUpperCase() === phrase.toUpperCase() || t.title.toUpperCase() === phrase.toUpperCase()
  )?.icon
  return tutorialIcon || iconMap[phrase.toUpperCase()] || 'gesture'
}

/** Transforms a raw backend DetectionHistory row into a HistoryEntry for display. */
function transformBackend(item: {
  id: number;
  phrase: string;
  confidence: number;
  platform: string;
  ...
  const status: 'high' | 'med' | 'low' = confidence > 90 ? 'high' : confidence > 75 ? 'med' : 'low'

  // Backend returns UTC datetime string — parse and convert to local time
  const dt = new Date(item.timestamp.endsWith('Z') ? item.timestamp : item.timestamp + 'Z')
  const dateStr = dt.toLocaleDateString('en-GB')          // DD/MM/YYYY
  const timeStr = dt.toTimeString().split(' ')[0]          // HH:MM:SS
  const fullTime = `${dateStr} ${timeStr}`

  const tutorialMatch = TUTORIALS.find(
    t => t.phrase.toLowerCase() === item.phrase.toLowerCase() || t.title.toLowerCase() === item.phrase.toLowerCase()
  )

  return {
    id: String(item.id),
    time: fullTime,
    gesture: tutorialMatch ? tutorialMatch.title : item.phrase,
    icon: getIcon(item.phrase),
    phrase: `"${tutorialMatch ? tutorialMatch.phrase : item.phrase}"`,
    confidenceStr: `${Math.round(confidence)}%`,
    platform: item.platform || 'Webcam',
    duration: '1.2s',
    status,
...
export default function History() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        const entries: HistoryEntry[] = (rawData as any[]).map(transformBackend)
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Date & Time', 'Gesture', 'Phrase', 'Confidence', 'Platform', 'Duration']
  ...
  const getMostUsed = () => {
    ...
    historyData.forEach(entry => {
      ...
      if (!freq[g]) freq[g] = { count: 0, icon: entry.icon }
      freq[g].count += 1
    })
    let maxKey = ''
    let maxCount = -1
    ...
    return { gesture: maxKey, count: maxCount, icon: freq[maxKey]?.icon || 'gesture' }
  }
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-surface-container-low p-6 rounded-xl flex flex-col justify-between border border-outline-variant/10 shadow-xl shadow-blue-500/5">
            ...
            <div className="mt-4 text-sm text-on-surface-variant flex items-center gap-1">
              {loading ? (
                ...
                <><span className="text-[#25D366] font-bold">YOUR</span> personal detections</>
              ) : (
          ...
          <div className="bg-surface-container-low p-6 rounded-xl flex flex-col justify-between border border-outline-variant/10 shadow-xl shadow-blue-500/5">
            <div>
              ...
              <h3 className="text-4xl font-extrabold mt-2 text-on-surface">{loading ? '—' : `${avgConfidence}%`}</h3>
            </div>
          ...
          <div className="bg-surface-container-low p-6 rounded-xl flex flex-col justify-between border border-outline-variant/10 shadow-xl shadow-blue-500/5">
            <div>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <h3 className="text-3xl font-extrabold text-on-surface capitalize">{loading ? '—' : mostUsed.gesture}</h3>
              </div>
            </div>
            <div className="mt-4 text-sm text-on-surface-variant italic">{mostUsed.count > 0 ? `${mostUsed.count} occurrences` : ''}</div>
          </div>
        </section>
              ...
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-on-surface-variant italic">
                    <span className="animate-pulse">Loading your history…</span>
                  </td>
                </tr>
              ) : error ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-error italic">
                    {error}
                  </td>
                </tr>
                ...
                currentEntries.map((row, idx) => (
                  <tr
                    key={row.id || idx}
                    className="bg-surface-container-lowest/20 hover:bg-surface-container-high/40 transition-colors group"
                  >
                    <td className="px-6 py-4 text-sm font-medium text-on-surface whitespace-nowrap">{row.time}</td>
          ...
          {!loading && totalDetections > 0 && (
            <div className="px-6 py-4 flex justify-between items-center text-sm border-t border-outline-variant/10">
              <span className="text-on-surface-variant italic text-xs">
                Showing {(page - 1) * itemsPerPage + 1}–{Math.min(page * itemsPerPage, totalDetections)} of{' '}
