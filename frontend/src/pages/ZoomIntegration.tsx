...
const PLATFORMS = [
  {
    ...
    stepUrl: 'app.zoom.us',
    stepLabel: 'Open Zoom Web',
    icon: '📹',
    storageKey: 'zoomBridge',
    urlBarText: 'app.zoom.us/wc/join/8482...',
  },
  {
    id: 'meet',
    name: 'Google Meet',
    ...
    stepUrl: 'meet.google.com',
    stepLabel: 'Open Google Meet',
    icon: '🟢',
    storageKey: 'googleMeet',
    urlBarText: 'meet.google.com/abc-defg-hij',
  },
  {
    id: 'teams',
    ...
    border: 'rgba(98,100,167,0.3)',
    url: 'https://teams.microsoft.com',
    stepUrl: 'teams.microsoft.com',
    stepLabel: 'Open Teams (Web)',
    icon: '🔵',
    storageKey: 'teamsBridge',
    urlBarText: 'teams.microsoft.com/v2/',
  },
];
  ...
  useEffect(() => {
    ...
    if (isTyping) {
      if (text.length < currentPhrase.text.length) {
        ...
        setIsTyping(false);
        const timeout = setTimeout(() => { setIsVisible(false); }, PAUSE_DURATION);
        return () => clearTimeout(timeout);
      }
    } else if (!isVisible) {
      const timeout = setTimeout(() => {
  ...
  return (
    <AnimatePresence>
      {isVisible && (
        ...
        >
          <div className="w-3 h-3 rounded-full" style={{ backgroundColor: phrase.color, boxShadow: `0 0 10px ${phrase.color}` }} />
...
export default function VideoCallIntegrations() {
  ...
  // Reset step progress when platform changes
  useEffect(() => {
    setActiveStep(1);
    setCompletedSteps([]);
  }, [activePlatform]);
  ...
  useEffect(() => {
    ...
    const checkHealth = async () => {
      try {
        ...
        if (isMounted) setBackendStatus(res.ok ? 'connected' : 'disconnected');
      } catch { if (isMounted) setBackendStatus('disconnected'); }
    };
    checkHealth();
  ...
  const completeStep = (stepNumber: number) => {
    if (!completedSteps.includes(stepNumber)) setCompletedSteps(prev => [...prev, stepNumber]);
    ...
    <div className="flex-1 w-full flex flex-col p-8 overflow-y-auto custom-scrollbar pt-20">
      ...
      <header className="mb-10">
        <div className="flex items-center gap-3 text-primary mb-2">
          <Video size={28} />
          <h1 className="text-3xl font-bold tracking-tight text-white">Video Call Integrations</h1>
        </div>
        <p className="text-on-surface-variant text-lg">Your signs become real-time subtitles on any video call platform.</p>
      </header>
      ...
      <div className="flex gap-3 mb-8 flex-wrap">
        {PLATFORMS.map((p, i) => (
          <button
            ...
            className="relative px-5 py-3 rounded-xl font-bold text-sm transition-all border"
            style={{
              background: activePlatform === i ? p.bg : 'transparent',
              borderColor: activePlatform === i ? p.border : 'rgba(255,255,255,0.08)',
              color: activePlatform === i ? p.color : '#94a3b8',
            }}
          >
            {p.icon} {p.name}
            {activePlatform === i && (
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
            ...
            <div className="lg:col-span-2 bg-surface-container-high rounded-2xl p-6 border border-white/5 flex flex-col justify-center relative overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                {backendStatus === 'checking' && (<><div className="w-4 h-4 rounded-full bg-yellow-500 animate-pulse shadow-[0_0_15px_rgba(234,179,8,0.5)]" /><p className="text-white font-medium">Checking connection...</p></>)}
                {backendStatus === 'connected' && (<><div className="w-4 h-4 rounded-full bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]"><div className="w-full h-full rounded-full bg-emerald-500 animate-ping opacity-75"></div></div><p className="text-emerald-400 font-medium">Synced — ready to broadcast to {platform.name}</p></>)}
                {backendStatus === 'disconnected' && (<><div className="w-4 h-4 rounded-full bg-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.5)]" /><div className="flex flex-col"><p className="text-rose-400 font-medium">Not running — start the backend first</p><code className="text-xs bg-black/40 px-2 py-1 rounded text-rose-200 mt-2 font-mono border border-rose-500/20">python3 -m uvicorn main:app --host 0.0.0.0 --port 10000</code></div></>)}
              </div>
            </div>
            ...
            <div className="bg-surface-container-high rounded-2xl p-6 border border-primary/20">
              ...
              <ul className="space-y-3 text-sm text-on-surface-variant font-medium">
                ...
                <li className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-primary rounded-full"></span> Works on: Zoom · Google Meet · Teams</li>
              </ul>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-10 mb-10">
            ...
            <div className="bg-[#1e1e1e] rounded-xl overflow-hidden border border-[#333] shadow-2xl flex flex-col h-[400px]">
              ...
              <div className="flex-1 bg-[#1a1a1a] p-4 relative">
                <div className="grid grid-cols-2 gap-4 h-full">
                  ...
                  <div className="bg-[#2a2a2a] rounded-lg border-2 relative overflow-hidden flex items-center justify-center" style={{ borderColor: platform.color }}>
                    ...
                    <div className="absolute bottom-2 left-2 flex gap-1">
                      {[1,2,3,4].map(i => <div key={i} className="w-1.5 h-3 rounded-sm" style={{ backgroundColor: platform.color }}></div>)}
                    </div>
                  </div>
                </div>
                <TypingSubtitle />
            ...
            <div className="flex flex-col justify-center gap-4">
              <h3 className="text-xl font-bold text-white mb-2">How It Works on {platform.name}</h3>
              ...
              <div className="bg-surface-container rounded-xl p-5 flex items-start gap-4 border border-white/5 hover:border-primary/30 transition-colors">
                <div className="p-3 rounded-lg" style={{ background: `${platform.color}20`, color: platform.color }}><MessageSquare size={24}/></div>
                <div>
                  <h4 className="font-bold text-white">3. Subtitle appears</h4>
                  <p className="text-sm text-on-surface-variant mt-1">The Chrome Extension injects a floating subtitle directly into your active {platform.name} call.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Setup Guide */}
          <div className="mb-10">
            <h3 className="text-xl font-bold text-white mb-6">Setup Guide for {platform.name}</h3>
            <div className="space-y-4">
                ...
                <div className="flex justify-between items-start">
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-on-surface-variant mt-1">Get the official SIGNETRA Chrome Extension — works on Zoom, Google Meet, and Teams.</p>
                      {activeStep === 1 && (
                        <button
                  ...
                  <div className="flex-1">
                    ...
                    {activeStep === 2 && (
                      <div className="mt-4 bg-black/30 p-4 rounded-lg space-y-3">
                        ...
                        <button onClick={() => completeStep(2)} className="mt-4 px-4 py-2 bg-surface-container-highest hover:bg-white/10 text-white font-medium text-sm rounded-lg border border-white/10 transition-colors">
                          I have installed it
                        </button>
                      </div>
                    )}
              ...
              <div className={`p-6 rounded-xl border-2 transition-colors ${activeStep === 3 ? 'border-primary bg-primary/5' : completedSteps.includes(3) ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/5 bg-surface-container-high'}`}>
                <div className="flex gap-4">
                  ...
                  <div>
                    <h4 className={`font-bold text-lg ${activeStep === 3 ? 'text-primary' : 'text-white'}`}>{platform.stepLabel}</h4>
                    <p className="text-sm text-on-surface-variant mt-1">Join or start a meeting on <span className="font-semibold" style={{ color: platform.color }}>{platform.stepUrl}</span> in your browser.</p>
                    {activeStep === 3 && (
                      <div className="mt-4 flex gap-3">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center gap-2 px-5 py-2.5 text-white font-bold rounded-lg hover:opacity-90 transition-all"
                          style={{ backgroundColor: platform.color }}
                        >
                          Open {platform.name} <ExternalLink size={16}/>
                        </a>
                        <button onClick={() => completeStep(3)} className="px-4 py-2 bg-transparent text-primary font-medium text-sm rounded-lg hover:bg-primary/10 transition-colors">
                          Mark Done
                        </button>
                      </div>
                    )}
